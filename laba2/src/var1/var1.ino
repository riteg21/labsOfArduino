...
// Время удержания кнопки для ночного режима
uint32_t timerPressStart = 0;
uint32_t timerPressEnd = 0;  
const int NIGHT_TOGGLE_HOLD_TIME = 5000;

// Для мигающего режима
...
String stateToString(State s) {
  switch (s) {
    case S_GREEN: return "ЗЕЛЁНЫЙ";
    case S_YELLOW: return "ЖЁЛТЫЙ";
    case S_RED: return "КРАСНЫЙ";
    case S_WARNING: return "ВНИМАНИЕ";
  }
  return "НЕИЗВЕСТНО";
}
...
void goToState(State newState, unsigned long dur, const char* reason) {
...
void readInputs() {
  ...
  if (buttonState == LOW && timerPressStart == 0 && (now - lastButtonPressTime > DEBOUNCE_DELAY)) {
    timerPressStart = now;
    lastButtonPressTime = now;
  } 
  else if (buttonState == HIGH && timerPressStart != 0 && (now - lastButtonPressTime > DEBOUNCE_DELAY)) {
    timerPressEnd = now;
    lastButtonPressTime = now;

    unsigned long pressDuration = timerPressEnd - timerPressStart;

    if (pressDuration >= NIGHT_TOGGLE_HOLD_TIME) {
      night_mode_toggle = !night_mode_toggle;
      Serial.print("[");
      Serial.print(now);
      Serial.println("мс] Ночной режим переключен");
    } 
    else {
      pedRequest = true;
      Serial.print("[");
      Serial.print(now);
      Serial.println("мс] Нажата кнопка пешехода");
    }
  ...
  if (emergencyState == LOW && !emergencyButtonPressed && (now - lastEmergencyPressTime > DEBOUNCE_DELAY)) {
    ...
    Serial.print("[");
    Serial.print(now);
    Serial.print("мс] Аварийный режим ");
    Serial.println(emergency ? "ВКЛ" : "ВЫКЛ");
  } 
  else if (emergencyState == HIGH && emergencyButtonPressed && (now - lastEmergencyPressTime > DEBOUNCE_DELAY)) {
...
void setup() {
  ...
  Serial.begin(9600);
  goToState(S_GREEN, 10000, "Начальный статус");
}
...
void loop() {
  ...
  // --- Режим предупреждения (ночь или авария) ---
  if (emergency || night_mode_toggle) {
    if (currentState != S_WARNING)
      goToState(S_WARNING, 500, emergency ? "Аварийный режим ВКЛ" : "Аварийный режим ВЫКЛ");
  }
  ...
  switch (currentState) {
    case S_GREEN:
      if (millis() - stateStart >= duration) {
        if (pedRequest) {
          goToState(S_YELLOW, 3000, "Запрос пешехода");
        } else {
          goToState(S_YELLOW, 3000, "Истёк таймер");
        }
      }
      break;
    ...
    case S_YELLOW:
      if (millis() - stateStart >= duration) {
        if (pedRequest) {
          pedRequest = false;
          goToState(S_RED, 15000, "Запрос пешехода (удлинённый красный)");
        } else {
          goToState(S_RED, 10000, "Истёк таймер");
        }
      }
      break;
    ...
    case S_RED:
      if (millis() - stateStart >= duration) {
        goToState(S_GREEN, 10000, "Истёк таймер");
      }
      break;
    ...
    case S_WARNING:
      ...
      if (!emergency && !night_mode_toggle)
        goToState(S_RED, 10000, "Аварийный/Ночной режим ВЫКЛ");
      break;
  }
}
...
\begin{center}
  \includegraphics[width = 1\textwidth]{blockshema1.png}
\end{center}

\textbf{Задание 2:} \\
Разработать и отладить конечный автомат (КДА) на платформе
Arduino, моделирующий управление светофором автомобильного перекрёстка  с использованием двумерной таблицы (состояние
× событие), где каждая ячейка содержит обработчик для комбинации
состояние/событие.
